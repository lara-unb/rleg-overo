...
void actuate(int spi_dev, MRA_DATA_STRUCT *mra_data)
{
  ...
  else if( (gpio_write(GPIO_CS_S3,1)==FAILURE) || (gpio_write(GPIO_CS_S2,0)==FAILURE) || (gpio_write(GPIO_CS_S1,1)==FAILURE) || (gpio_write(GPIO_CS_S0,0)==FAILURE) )
	mra_data->new_ctl=FAILURE;
